...
exports.sendEmailChangeOtp = async (req, res) => {
  try {
    ...
    await redisClient.setEx(emailKey, 420, otp);
...
exports.updatePassword = async (req, res) => {
  try {
    ...
    const isMatched = bcryptjs.compare(oldPassword, userInfo.password);

    if (!isMatched) {
      return res.status(400).json({ message: 'Old password not matchedd', success: false });
    }
    ...
    console.log('Error occured in update password fn updatePassword', error);
    res.status(500).json({ message: 'Server error', success: false });
  }
}
