...
exports.generateAccountSecurityCode = async (req, res) => {
  try {
    const user = req.user;
    const { accountType } = req.body; // e.g., "Instagram", "Facebook", "Youtube"

    if (!user) {
      return res.status(401).json({ message: 'User not authorised', success: false });
    }

    if (!accountType || !['Instgram', 'Youtube', 'Facebook'].includes(accountType)) {
    ...
    res.status(201).json({
      ...
      success: true,
      verificationCode: generatedSecurityKey, // only send in dev; hide in prod
    });
