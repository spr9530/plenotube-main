...
const { OTPQueue } = require('./worker.js');
...
const generateUniqueUsername = async (fullName) => {
    const firstPart = fullName?.trim().split(" ")[0].toLowerCase();
    let username;
    let isUnique = false;

    while (!isUnique) {
        const randomDigits = Math.floor(100000 + Math.random() * 900000); // 6 random digits
        username = `${firstPart}${randomDigits}`;
...
exports.createNewAccount = async (req, res) => {
    try {
        ...
        if (emailAttempts && parseInt(emailAttempts) >= 10) {
        ...
        if (ipAttempts && parseInt(ipAttempts) >= 50) {
            return res.status(429).json({
                success: false,
...
exports.createAccountViaGoogle = async (req, res) => {
    try {
        ...
        if (existing && existing.isVerified) {
            ...
            return res.status(200).json({ success: true, message: "User already exists", user: existing, token });
        }
