...
exports.verifySignUpOtp = async (req, res) => {
    try {
        ...
        if (!email) {
            return res.status(400).json({ success: false, session: false, message: 'Your session has expired please try again' })
        }
        ...
        if (cacheOtp) {
            ...
            if (otp === cacheOtp) {
                ...
                const cacheUser = JSON.parse(redisClient.get(`signup:${email}`));
                await Promise.all([
                    User.save(cacheUser),
                    OTP.deleteOne({ email }),
