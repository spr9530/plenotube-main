...
function App() {
  ...
  const [isAuthenticated, setIsAuthenticated] = useState(user && token ? true : false);

  useEffect(() => {
    setIsAuthenticated(user && token ? true : false)
  }, [user, token])
  ...
  return (
    <Routes>
      ...
      <Route element={<ProtectedRoute isAuthenticated={isAuthenticated} redirect='platform'>
        <Platform />
