...
import { generateUserMailOtpApi, genrateSecurityKeyApi, getUserGeneralApi, updatePasswordApi, updateUserGeneralApi, verifyAndUpdateUserMailApi } from "../api/userApi";
...
const useUser = () => {
  ...
  // const genrateSecurityKey = useCallback(async() => {
  //   try{
  //     setLoading(true);
  //     const res = await genrateSecurityKeyApi();
  //     setLoading(false);
  //     return true;

  //   }catch(error){
  //     generateToast({tittle:'', message:'Something went wrong', type:'danger'});
  //     setLoading(false);
  //     return false;
  //   }
  // })

  return { getUserGeneral, updateUserGeneral, genrateMailChangeOtp, verifyOtp, genrateSecurityKey, updateUserPassword, user, loading };
};
