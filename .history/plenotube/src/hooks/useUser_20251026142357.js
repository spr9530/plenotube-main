...
import { genrateUserMailOtpApi, getUserGeneralApi, updatePasswordApi, updateUserGeneralApi, verifyAndUpdateUserMailApi } from "../api/userApi";
...
const useUser = () => {
  ...
  const genrateMailChangeOtp = useCallback(async (data) => {
    try {
      ...
      console.log(response)
      if (!response.success) {
        generateToast({title:'Mail Error', message:response.message, type:'error'})
      }
      console.log(response);
      setLoading(false);
    } catch (error) {
