...
function AccountModel({ modal, setModal, heading, setHeading,
    ...
    const linkIsValid = (platform) => {
        ...
        if (regex.test(link.trim())) {
            return { valid: true, message: 'Valid profile link' };
        } else {
            return { valid: false, message: `Invalid ${platform} profile URL` };
        }
    }


    useEffect(() => {
        if (link != '' && linkIsValid(platform)) {
            setShowSubmit(true);
        }
