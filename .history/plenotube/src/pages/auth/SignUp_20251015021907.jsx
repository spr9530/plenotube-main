...
function SignUp() {
    ...
    const { handleRegister, handleRegisterViaGoogle, handleOtp, isOtp, user, loading: authLoading, error: authError } = useAuth();
    const handleSuccess = async (credentialResponse) => {
        const decoded = jwtDecode(credentialResponse.credential);
        const { email, name } = decoded;
        handleRegisterViaGoogle({ name, email })
    ...
    return (
        <DefaultLayout>
            <div className='h-full w-full flex items-center justify-center p-2'>
                {
                    isOtp ?
                        <form onSubmit={handleSubmit(submitOtp)} className='w-full h-full flex justify-center'>
