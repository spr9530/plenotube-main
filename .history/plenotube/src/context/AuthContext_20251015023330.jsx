...
import { checkOtpApi, registerUserApi, registerViaGoogleApi } from "../api/authApi";
...
export const AuthProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        if (state.token && state.user) {
            ...
            localStorage.setItem("otp", JSON.stringify({ user: state.user }));
        }
        else {
    ...
    const handleOtp = useCallback(async (data) => {
    ...
    const handleLogin = useCallback(async (credentials) => {
        const path = URL.params.redirect || '/discover';
        try {
    ...
    return (
        <AuthContext.Provider value={{ ...state, isOtp, handleRegister, handleRegisterViaGoogle, handleLogout, handleOtp, handleLogin }}>
            {children}
