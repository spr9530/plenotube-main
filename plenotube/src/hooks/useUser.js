...
import { generateUserMailOtpApi, genrateSecurityKeyApi, getUserGeneralApi, submitSocialProfileApi, updatePasswordApi, updateUserGeneralApi, verifyAndUpdateUserMailApi } from "../api/userApi";
...
const useUser = () => {
  ...
  const updateUserPassword = useCallback(async (data) => {
    try {
      ...
      generateToast({ title: 'Password Updated', message: response.message, type: 'success' })
      setLoading(false);
      return true;
    } catch (error) {
      generateToast({ title: 'Mail Error', message: error.message, type: 'danger' })
  ...
  const genrateAccountSecurityKey = useCallback(async(platform) =>{
    try{
      ...
      return response.verificationCode;
       ...
       generateToast({ title: 'Account Error', message: error.message, type: 'danger' })
      return false;
    }
  },[user])
