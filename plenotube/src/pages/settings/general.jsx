...
function General() {
    ...
    const handleGenrateOtp = async (e) => {
        e.preventDefault();
        if (email != '') {
            const res = await genrateMailChangeOtp({ email });
            console.log(res)
            setIsVerify(res);
        } else {
    ...
    const verifyEmialChange = async (e) => {
        console.log('calld')
        e.preventDefault();
        const res = await verifyOtp({ newemail: email, email: user?.email, otp: emailOtp })
        if (res) {
            setEmailOtp('');
            setEditEmail(false);
            setEmail(user?.email);
    ...
    const handlePassword = async (e) => {
        ...
        if ((user?.password && password !== '') || newPassword !== '') {
            const res = await updateUserPassword({ oldPassword: password || 'new', newPassword: newPassword })
            if (res) {
                setPassword('');
                setNewPassword('');
            ...
            generateToast({ title: 'Password', message: "Password can't be empty", type: 'warning' })
        }
    }
                    ...
                    <div className='flex flex-col gap-8 h-full max-h-100 overflow-auto custom-scroll'>
                                ...
                                {editEmail ? (
                                    <>
                                        <Button
                                            ...
                                            isDisabled={isInvalid || !email} // prevents click when invalid or empty
                                            onClick={(e) => {
                                                e.preventDefault();
                                                if (isVerify) {
                                                    verifyEmialChange(e);
                                        ...
                                        <Button
                                            ...
                                            color="default"
                                            onClick={() => {
                                                setEmail(user?.email); // reset properly even if undefined
                                                setIsVerify(false);          // reset state if you’re in verify mode
                                                setEditEmail(false);
                                                setEmailOtp('');
                                            }}
                                        >
                        ...
                        <div className='flex justify-between items-center w-full gap-3'>
                            ...
                            <div className='flex gap-2 mt-7'>
                                ...
                                {editPassword ? <>
                                    <Button isIconOnly variant='flat' size='lg' color='primary' isLoading={userLoading} onClick={(e) => handlePassword(e)}>
                                        <RiCheckDoubleFill />
                                    </Button>
                                    ...
                                    <Button isIconOnly variant='light' size='lg' color='primary' className='text-xl' onClick={() => {
